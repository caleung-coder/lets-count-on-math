...
const CONCEPT = "which_group_has_more"
const TEMPLATE_KEY = "which_group_has_more"
...
function explanationFor(
  ...
  if (difference === 1) {
    ...
    if (firstCount > secondCount) {
      return `There is 1 more ${singularize(firstNamePlural)} than ${secondNamePlural}.`
    }

    return `There is 1 more ${singularize(secondNamePlural)} than ${firstNamePlural}.`
  }

  if (firstCount > secondCount) {
    return `There are ${difference} more ${firstNamePlural} than ${secondNamePlural}.`
  }

  return `There are ${difference} more ${secondNamePlural} than ${firstNamePlural}.`
}

export function generateWhichGroupHasMoreQuestion(
